...
def create_model(model_hyperparams):
    class GNN_CrossAttention_CORAL(torch.nn.Module):
    ...
    return GNN_CrossAttention_CORAL(
    ...
    for run_id in tqdm(range(hyper_params['num_splits']), 'Splits training'):
        ...
        for epoch in range(num_epochs):
            ...
            for country in countries_data:
                ...
                features_dict[country] = features[train_mask]
            ...
            # 2. Pairwise CORAL Covariance Alignment across all source countries
            coral_val = 0
            ...
            for i in range(num_c):
                for j in range(i + 1, num_c):
                    c1 = country_list[i]
                    c2 = country_list[j]
                    coral_val += coral_loss(features_dict[c1], features_dict[c2])
                    pair_count += 1
                    
            if pair_count > 0:
                coral_val = coral_val / pair_count
                
            # Add weighted CORAL loss to align representations
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run GNN model with CORAL Alignment")
