...
from model_eval import TrainLogMetrics, TestLogMetrics, eval_pred, get_best_threshold
...
# Decision thresholds swept during calibration on the target validation split.
CALIBRATION_THRESHOLDS = np.linspace(0.05, 0.95, 19)
...
def calibrate_threshold(prob, labels, mask, metric_to_optimize='f1_macro'):
    """Pick the decision threshold maximising `metric_to_optimize` on `mask`.

    Source batches are class-balanced (64/64) so the model outputs probabilities
    calibrated to a 50/50 prior, while target countries are highly imbalanced
    (e.g. Cuba 2.3% IO). A fixed 0.5 cut is therefore mis-matched; we re-use the
    existing `get_best_threshold` sweep (model_eval.py) over the target validation
    split -- the same split already used for early stopping.
    """
    preds_by_threshold = [prob > t for t in CALIBRATION_THRESHOLDS]
    best_idx = get_best_threshold(labels, preds_by_threshold, mask, metric_to_optimize)
    return CALIBRATION_THRESHOLDS[best_idx]
...
def create_model(model_hyperparams):
    class GNN_CrossAttention_CALDFA(torch.nn.Module):
        def __init__(self, num_node_features, hidden_dim, num_classes, num_textual_features, num_structural_features,
                     activation_fn=torch.nn.ReLU(), dropout_p=0.2, gnn_type='gcn', use_layernorm=True):
            super().__init__()
            ...
            # MODIFICATION (CAL-DFA): LayerNorm after each modality projector. LayerNorm is a
            # per-node, across-feature normalisation with NO running statistics, so it leaks no
            # source-domain stats into the target and puts every country's text/structure
            # features on the same scale -- this is meant to suppress cross-country scale drift.
            struct_layers = [torch.nn.Linear(num_structural_features, hidden_dim), torch.nn.ReLU()]
            text_layers = [torch.nn.Linear(num_textual_features, hidden_dim), torch.nn.ReLU()]
            if use_layernorm:
                struct_layers.append(torch.nn.LayerNorm(hidden_dim))
                text_layers.append(torch.nn.LayerNorm(hidden_dim))
            self.struct_projector = torch.nn.Sequential(*struct_layers)
            self.text_projector = torch.nn.Sequential(*text_layers)
            self.joint_projector = torch.nn.Sequential(
                torch.nn.Linear(hidden_dim * 2, hidden_dim * 2),
                torch.nn.ReLU()
            )
            # MODIFICATION (CAL-DFA): normalise the fused representation before message passing.
            self.fused_norm = torch.nn.LayerNorm(hidden_dim * 2) if use_layernorm else torch.nn.Identity()
    ...
    return GNN_CrossAttention_CALDFA(
        ...
        use_layernorm=model_hyperparams.get('use_layernorm', True)
    )
...
def main(dataset_name, train_hyperparams, model_hyperparams, hyper_params, device_id):
    ...
    coral_loss_weight = train_hyperparams['coral_weight']
    calibrate = train_hyperparams['calibrate']
    ...
    for run_id in tqdm(range(hyper_params['num_splits']), 'Splits training'):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % train_hyperparams["check_loss_freq"] == 0:
                ...
                with torch.no_grad():
                    ...
                    # MODIFICATION (CAL-DFA): optionally select on the calibrated-threshold val F1 so
                    # model selection is consistent with the calibrated final evaluation.
                    val_thr = calibrate_threshold(pred, numpy_labels, datasets['splits'][run_id]['val'],
                                                  metric_to_optimize) if calibrate else 0.5
                    val_metrics = eval_pred(numpy_labels, pred > val_thr, datasets['splits'][run_id]['val'])
                    ...
                    print(
                        f'Epoch {epoch}/{num_epochs} train_loss: {loss.item():.4f} (task_loss: {task_loss_sum:.4f}, coral_loss: {coral_loss_sum:.4f}) -- val_{metric_to_optimize}: {val_metrics[metric_to_optimize]:.4f} (thr={val_thr:.2f})')
            else:
        ...
        with torch.no_grad():
            pred = model(node_features, struct_node_features, edge_index).detach().cpu().numpy().flatten()
        # MODIFICATION (CAL-DFA): optionally calibrate the decision threshold on the target val
        # split, then apply the SAME threshold to every test subset below.
        best_thr = calibrate_threshold(pred, numpy_labels, datasets['splits'][run_id]['val'],
                                       metric_to_optimize) if calibrate else 0.5
        print(f'[Split {run_id}] decision threshold = {best_thr:.2f} (calibrate={calibrate})')
        pred_label = pred > best_thr
        # Evaluate performance on val set
        val_metrics = eval_pred(numpy_labels, pred_label, datasets['splits'][run_id]['val'])
        for metric_name in val_metrics:
            val_logger.update(metric_name, run_id, val_metrics[metric_name])
        # Evaluate performance on test set
        test_metrics = eval_pred(numpy_labels, pred_label, datasets['splits'][run_id]['test'])
        for metric_name in test_metrics:
            test_logger.update(metric_name, run_id, test_metrics[metric_name])
        # Evaluate performance on test set (only coRT nodes)
        test_metrics_coRT = eval_pred(numpy_labels, pred_label,
        ...
        test_metrics_coURL = eval_pred(numpy_labels, pred_label,
        ...
        test_metrics_hashSeq = eval_pred(numpy_labels, pred_label,
        ...
        test_metrics_fastRT = eval_pred(numpy_labels, pred_label,
        ...
        test_metrics_tweetSim = eval_pred(numpy_labels, pred_label,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run GNN model with Calibrated Decoupled Feature Alignment (CAL-DFA)")
    ...
    parser.add_argument('-coral_weight', '--coral_weight', type=float,
                        help='Weight of the text-feature CORAL alignment loss (lambda)', default=1.0)
    parser.add_argument('--calibrate', action='store_true',
                        help='[ablation] Calibrate the decision threshold on the target val split. '
                             'Empirically harmful for rare-positive targets; default OFF (fixed 0.5).')
    parser.add_argument('--use_layernorm', type=int, default=0,
                        help='[ablation] Add LayerNorm feature normalisation (1=on, 0=off). '
                             'Empirically harmful (Cuba/China collapse); default OFF.')
    ...
    train_hyperparameters = {'num_epochs': args.epochs, 'learning_rate': args.lr,
                             ...
                             'metric_to_optimize': args.val_metric, 'coral_weight': args.coral_weight,
