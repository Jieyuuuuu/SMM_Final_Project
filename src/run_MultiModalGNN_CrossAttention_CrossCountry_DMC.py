...
# =============================================================================
# Focal Loss (targets target imbalance)
# =============================================================================
class FocalLoss(torch.nn.Module):
    def __init__(self, gamma=2.0, alpha=0.75, reduction='mean'):
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha  # Weight for positive (IO) class
...
# =============================================================================
# GNN_CrossAttention_DMC Model Definition
# =============================================================================
class GNN_CrossAttention_DMC(torch.nn.Module):
    def __init__(self, num_node_features, hidden_dim, num_classes, num_textual_features,
        ...
        # 5 dedicated channels for each behavioral subnet
        ...
        # Learnable softmax channel fusion weights (initialized to equal weights)
        self.channel_weights = torch.nn.Parameter(torch.zeros(5))
    ...
    def forward(self, text_node_features, struct_node_features, edge_indices,
        ...
        # Pass through individual channels
        ...
        # Compute Softmax channel attention weights
        ...
        fused = (
            ...
            weights[2] * z_hashSeq +
            weights[3] * z_fastRT +
            weights[4] * z_tweetSim
        )

        out = torch.exp(self.output_fn(self.classifier(fused)))

        if return_text_features:
            return out, text_projection
        return out


# =============================================================================
# Multi-channel data loader with stable global ID mapping preservation
# =============================================================================
def read_all_data_dmc(device_id, dataset_name, hyper_params, train_hyperparams, model_hyperparams):
    ...
    # 1. Capture the global node set to ensure sequential mapping consistency
    _, global_network = handle_isolated_nodes(copy.deepcopy(datasets['graph']))
    global_nodes = list(global_network.nodes())

    # 2. Extract edge indices for all 5 subnets separately, preserving the global mapping
    ...
    for subnet_name in subnets:
        ...
        if (data_dir / fname).exists():
            ...
            subnet_graph = datasets[subnet_name]
...
def main(dataset_name, train_hyperparams, model_hyperparams, hyper_params, device_id):
    ...
    device, base_dir, interim_data_dir, data_dir, datasets, edge_indices, network, \
        node_features, struct_node_features = read_all_data_dmc(
    ...
    # Preload source country datasets
    other_countries = [c for c in ALL_COUNTRIES if c != dataset_name]
    countries_data = {}
    countries_numExamples = {}
    for country in other_countries:
        _, _, _, _, c_datasets, c_edge_indices, _, c_node_features, c_struct_node_features = read_all_data_dmc(
    ...
    for run_id in tqdm(range(hyper_params['num_splits']), 'Splits training'):
        ...
        for epoch in range(num_epochs):
            ...
            # Pairwise CORAL Covariance Alignment strictly on decoupled text projection
            coral_val = 0
        ...
        # Load best snapshot for final evaluation
        model.load_state_dict(torch.load(best_model_path, map_location=device))
        model.eval()
        with torch.no_grad():
            pred = model(node_features, struct_node_features, edge_indices).detach().cpu().numpy().flatten()
            final_weights = torch.softmax(model.channel_weights, dim=0).cpu().numpy()
            print(f"\n[Split {run_id}] Learned Channel Weights: "
                  f"coRT={final_weights[0]:.4f}, coURL={final_weights[1]:.4f}, "
                  f"hashSeq={final_weights[2]:.4f}, fastRT={final_weights[3]:.4f}, "
                  f"tweetSim={final_weights[4]:.4f}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run Decoupled Multi-Channel GFM (DMC-GFM) model")
