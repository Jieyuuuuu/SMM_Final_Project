...
class NodeLevelAttentionFusion(torch.nn.Module):
    ...
    def forward(self, z_list):
        # z_list: list of 5 tensors, each shape [N, hidden_dim]
        # Stack to [N, 5, hidden_dim]
        z_stack = torch.stack(z_list, dim=1)
        # Compute scores: [N, 5, 1]
        scores = self.attn_proj(z_stack)
        # Softmax over the 5 channels: [N, 5, 1]
        weights = torch.softmax(scores, dim=1)
        # Weighted fusion: [N, hidden_dim]
        z_fused = torch.sum(weights * z_stack, dim=1)
...
class GNN_CrossAttention_AMC(torch.nn.Module):
    def __init__(self, num_node_features, hidden_dim, num_classes, num_textual_features,
        ...
        # Dynamic Node-Level Attention Fusion
...
def read_all_data_amc(device_id, dataset_name, hyper_params, train_hyperparams, model_hyperparams):
    ...
    # Optimized: Use graph.copy() instead of extremely slow copy.deepcopy()
...
def main(dataset_name, train_hyperparams, model_hyperparams, hyper_params, device_id):
    ...
    for country in other_countries:
        _, _, _, _, c_datasets, c_edge_indices, _, c_node_features, c_struct_node_features = read_all_data_amc(
    ...
    for run_id in tqdm(range(hyper_params['num_splits']), 'Splits training'):
        ...
        for epoch in range(num_epochs):
            ...
            # 1. Forward pass on source countries
            ...
            # 2. Project target country text features for domain adaptation (extremely fast, no target GNN forward pass)
            ...
            # 3. Compute target-aligned CORAL loss (covariance alignment between all source countries and the target)
            coral_val = 0
            for country in countries_data:
                coral_val += coral_loss(text_features_dict[country], target_text_feats)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run Adaptive Multi-Channel GFM (AMC-GFM) model")
