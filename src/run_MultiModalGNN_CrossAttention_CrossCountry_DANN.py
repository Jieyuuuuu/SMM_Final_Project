...
from models_dann import GNN_CrossAttention_DANN, DomainClassifier
...
def create_model(model_hyperparams):
    return GNN_CrossAttention_DANN(
...
def main(dataset_name, train_hyperparams, model_hyperparams, hyper_params, device_id):
    ...
    # Setup country index mappings for domain labels (0 to 4)
    country_to_domain_id = {country: idx for idx, country in enumerate(other_countries)}
    print("Country domain mappings:", country_to_domain_id)
    ...
    for run_id in tqdm(range(hyper_params['num_splits']), 'Splits training'):
        ...
        # Instantiate Domain Classifier
        domain_classifier = DomainClassifier(in_dim=model_hyperparams['latent_dim'] * 2, num_domains=len(other_countries))
        domain_classifier.to(device)

        # Create optimizer including parameters from both networks
        ...
        for epoch in range(num_epochs):
            ...
            # Dynamic adversarial factor lambda (scales from 0 to 1 as training progresses)
            p = float(epoch) / num_epochs
            alpha = float(2.0 / (1.0 + np.exp(-10 * p)) - 1)
            ...
            for country in countries_data:
                ...
                # Forward pass: get task predictions AND intermediate multimodal representations
                pred, features = model(countries_data[country]['node_features'],
                ...
                domain_preds = domain_classifier(features[train_mask], alpha=alpha)
                domain_targets = torch.full(
                    (domain_preds.size(0),), 
                    country_to_domain_id[country], 
                    dtype=torch.long, 
                    device=device
                )
                domain_loss = domain_loss_fn(domain_preds, domain_targets)
                
                # Joint training optimization with domain loss weight scaling
                domain_loss_weight = 0.05
                loss += task_loss + (domain_loss * domain_loss_weight)
            ...
            if epoch % train_hyperparams["check_loss_freq"] == 0:
                ...
                with torch.no_grad():
                    ...
                    print(
                        f'Epoch {epoch}/{num_epochs} train_loss: {loss.item():.4f} (task_loss: {task_loss_sum:.4f}, domain_loss: {domain_loss_sum:.4f}) -- val_{metric_to_optimize}: {val_metrics[metric_to_optimize]:.4f}')
            else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run GNN model with DANN")
